...
import org.meshtastic.proto.Portnums.PortNum.TEXT_MESSAGE_APP_VALUE
...
class MeshService : Service(), Logging {
    ...
    private fun rememberDataPacket(dataPacket: DataPacket, updateNotification: Boolean = true) {
        ...
        val compressed = dataPacket.dataType == Portnums.PortNum.TEXT_MESSAGE_COMPRESSED_APP_VALUE

        val readableText = if(compressed && dataPacket.bytes != null){
            NativeMessageCompression.decompressText(dataPacket.bytes) ?: dataPacket.text.orEmpty()
        } else {
            dataPacket.text.orEmpty()
        }

        val normalizedPacket = dataPacket.copy(
            clearText = readableText,
        ...
        serviceScope.handledLaunch {
            ...
            packetRepository.get().apply {
                ...
                val isChatMessage = compressed ||
                        normalizedPacket.dataType == TEXT_MESSAGE_APP_VALUE

                val chatIsFocused = isChatMessage && ActiveChatTracker.isChatActiveAndFocused(contactKey)
                ...
                if (updateNotification &&
                    ...
                    !chatIsFocused)
                {
                    updateMessageNotification(contactKey, normalizedPacket)
                }

                if(chatIsFocused){
                    clearUnreadCount(contactKey, now)
                }
            }
        }
    }
            ...
            if (dataPacket != null) {
                ...
                when (data.portnumValue) {
                    ...
                    Portnums.PortNum.NEIGHBORINFO_APP_VALUE -> {
                        ...
                        val ourRequestId = ourNeighborRequests.remove(
                            packet.decoded.requestId
                        )

                        val backgroundHunting = huntingPrefs.getBoolean(
                            UserPrefs.Hunting.BACKGROUND_HUNT, false
                        )

                        if(ourRequestId != null && !backgroundHunting){
                            //we are only interested in requests generated by this client
    ...
    private fun setOwner(packetId: Int, user: MeshProtos.User) = with(user) {
        ...
        val old = dest.user
        if (longName == old.longName && shortName == old.shortName && isLicensed == old.isLicensed && isUnmessagable == old.isUnmessagable) {
            debug("Ignoring nop owner change")
        } else {
            debug("setOwner Id: $id longName: ${longName.anonymize} shortName: $shortName isLicensed: $isLicensed isUnmessagable: $isUnmessagable")
